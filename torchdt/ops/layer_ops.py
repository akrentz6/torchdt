...
class DTDropoutFunction(DTFunction):
    ...
    def backward(ctx, ops, grad_output):
        ...
        grad_x = torch.where(
            output == ops.from_float(0.0),
            ops.from_float(0.0),
            ops.mul(grad_output, ops.from_float(1 / (1 - ctx.p))))
        return grad_x, None
...
def dt_conv2d(ops, x, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    if pad_h > 0 or pad_w > 0:
        x_padded = torch.nn.functional.pad(
            x, (pad_w, pad_w, pad_h, pad_h), value=ops.from_float(0.0).item()
        )
    else:
        x_padded = x
...
class DTConv2dFunction(DTFunction):
    ...
    def backward(ctx, ops, grad_output):
        ...
        if pad_h > 0 or pad_w > 0:
            x_padded = torch.nn.functional.pad(x, (pad_w, pad_w, pad_h, pad_h), value=ops.from_float(0.0).item())
        else:
            x_padded = x
...
def dt_avg_pool2d(ops, x, kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True, divisor_override=None):
    ...
    kernel_area_dt = ops.from_float(kernel_h * kernel_w)
            ...
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    if divisor_override is not None:
                        ...
                        divisor = ops.from_float(max(valid_count, 1))
        ...
        for c in range(C):
            for h_out in range(H_out):
                ...
                for w_out in range(W_out):
                    ...
                    sm = ops.sum(window)
                    divisor = ops.from_float(max((h_end - h_start) * (w_end - w_start), 1))
                    out[n, c, h_out, w_out] = ops.div(sm, divisor)
...
def dt_max_pool2d(ops, x, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False, return_indices=False):
    ...
    if pad_h > 0 or pad_w > 0:
        x_padded = torch.nn.functional.pad(x, (pad_w, pad_w, pad_h, pad_h),
                                           value=ops.from_float(float("-inf")).item())
    else:
        x_padded = x
...
def dt_batch_norm(ops, x, running_mean, running_var, momentum, eps, weight=None, bias=None, training=True):
    ...
    if training:
        ...
        batch_var = ops.var(x, ops.from_float(0.0), dim=red_dims, keepdim=True)
        batch_var_corrected = ops.var(x, ops.from_float(1.0), dim=red_dims, keepdim=True)

        with torch.no_grad():
            one_minus_momentum = ops.sub(ops.from_float(1.0), momentum)
    ...
    inv_std = ops.div(ops.from_float(1.0), ops.sqrt(var_eps))
...
class DTBatchNormFunction(DTFunction):
    ...
    def setup_context(ctx, ops, inputs, output):
        ...
        if training:
            ...
            var = ops.var(x, ops.from_float(1.0), dim=ctx.red_dims, keepdim=True)
        else:
            mean = running_mean.view(1, -1, *([1] * (x.dim() - 2)))
            var = running_var.view(1, -1, *([1] * (x.dim() - 2)))
    ...
    def backward(ctx, ops, grad_output):
        ...
        if ctx.training:
            ...
            one_over_N = ops.div(ops.from_float(1.0), n_elems)
...
def dt_layer_norm(ops, x, eps, normalized_shape, weight=None, bias=None):
    ...
    var = ops.var(x, ops.from_float(0.0), dim=reduce_dims, keepdim=True)
...
class DTLayerNormFunction(DTFunction):
    ...
    def setup_context(ctx, ops, inputs, output):
        ...
        mean = ops.mean(x, dim=ctx.red_dims, keepdim=True)
        var = ops.var(x, ops.from_float(0.0), dim=ctx.red_dims, keepdim=True)
        ctx.save_for_backward(x, weight, bias, mean, var, eps)
    ...
    def backward(ctx, ops, grad_output):
        ...
        inv_N = ops.div(ops.from_float(1.0), n_elems)
