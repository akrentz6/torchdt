...
def dt_sqrt(ops, x):
    return ops.pow(x, ops.from_float(0.5))
...
class DTSqrtFunction(DTFunction):
    ...
    def backward(ctx, ops, grad_output):
        ...
        grad_x = ops.div(grad_output, ops.mul(ops.from_float(2), output))
        return grad_x

@register_base_op("reciprocal")
def dt_reciprocal(ops, x):
    return ops.div(ops.from_float(1), x)
...
def dt_exp(ops, x):
    return ops.pow(ops.from_float(torch.e), x)
...
def dt_mean(ops, x, dim=None, keepdim=False):
    ...
    return ops.div(total, ops.from_float(n_elem))
...
class DTMeanFunction(DTFunction):
    ...
    def backward(ctx, ops, grad_output):
        ...
        grad_x = ops.div(grad_output, ops.from_float(n_elem))
        if dims is None:
            grad_x = grad_x.expand(x.shape)
...
class DTVarFunction(DTFunction):
    ...
    def backward(ctx, ops, grad_output):
        ...
        diff = ops.sub(x, mean)
        scale = ops.div(ops.from_float(2.0), denom)
